...
public class Camera1Manager extends BaseCameraManager<Integer, SurfaceHolder.Callback> implements SurfaceHolder.Callback, Camera.PictureCallback {
    ...
    public void closeCamera(final CameraCloseListener<Integer> cameraCloseListener) {
        if (backgroundHandler == null) {
            Log.e(TAG, "closeCamera: backgroundHandler is null");
            return;
        }
        backgroundHandler.post(new Runnable() {
            @Override
    ...
    public void takePhoto(File photoFile, CameraPhotoListener cameraPhotoListener) {
        ...
        if (backgroundHandler == null) {
            Log.d(TAG, "takePhoto: backgroundHandler IS NULL");
            return;
        }
        backgroundHandler.post(new Runnable() {
            @Override
    ...
    public int getCameraZoom() {
        return intCameraZoom;
    }

    @Override
    public void setCameraZoom(int zoom) {
        setCameraZoom(camera, camera.getParameters(), zoom);
    }
    ...
    protected void prepareCameraOutputs() {
        try {
            ...
            videoSize = CameraHelper.getSizeWithClosestRatio((videoSizes == null || videoSizes.isEmpty()) ? previewSizes : videoSizes, camcorderProfile.videoFrameWidth, camcorderProfile
                    .videoFrameHeight);

            photoSize = CameraHelper.getPictureSize((pictureSizes == null || pictureSizes.isEmpty()) ? previewSizes : pictureSizes, configurationProvider.getMediaQuality() == CameraConfiguration
                    .MEDIA_QUALITY_AUTO ? CameraConfiguration.MEDIA_QUALITY_HIGHEST : configurationProvider.getMediaQuality());

            if (configurationProvider.getMediaAction() == CameraConfiguration.MEDIA_ACTION_PHOTO || configurationProvider.getMediaAction() == CameraConfiguration.MEDIA_ACTION_BOTH) {
    ...
    private void startPreview(SurfaceHolder surfaceHolder) {
        try {
            ...
            if (Build.VERSION.SDK_INT > 13 && (configurationProvider.getMediaAction() == CameraConfiguration.MEDIA_ACTION_VIDEO || configurationProvider.getMediaAction() == CameraConfiguration
                    .MEDIA_ACTION_BOTH)) {
//                parameters.setRecordingHint(true);
            }

            if (Build.VERSION.SDK_INT > 14 && parameters.isVideoStabilizationSupported() && (configurationProvider.getMediaAction() == CameraConfiguration.MEDIA_ACTION_VIDEO ||
        ...
        try {
            switch (flashMode) {
                ...
                case CameraConfiguration.FLASH_MODE_ON:
                    if (currentCameraId.equals(faceFrontCameraId) && !CameraHelper.hasFrontCameraFlash(Camera1Manager.this.context)) {
                        parameters.setFlashMode(Camera.Parameters.FLASH_MODE_AUTO);
                        ;
                    } else
    ...
    private void setCameraZoom(Camera camera, Camera.Parameters parameters, int zoom) {
        try {
            if (zoom >= 0 && zoom < parameters.getMaxZoom() && parameters.isZoomSupported()) {
