...
public final class CameraHelper {
    ...
    public static boolean hasCamera(Context context) {
        Log.i(TAG, "hasCamera hasSystemFeature(PackageManager.FEATURE_CAMERA): " + context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA));
        Log.i(TAG, "hasCamera hasSystemFeature(PackageManager.FEATURE_CAMERA_FRONT): " + context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA_FRONT));
    ...
    public static boolean hasFrontCameraFlash(Context context) {
        ...
        try {
            ...
            if (idList.length == 0) {
                ...
                for (final String str : idList) {
                    ...
                    final CameraCharacteristics characteristics = manager.getCameraCharacteristics(str);
                    System.out.println(str + " " + characteristics.get(CameraCharacteristics.FLASH_INFO_AVAILABLE).booleanValue() + "");
                    // System.out.println( str + " " + characteristics.get(CameraCharacteristics.facede).booleanValue()+"");
                    int orientation = characteristics.get(CameraCharacteristics.LENS_FACING);
                    if (orientation == CameraCharacteristics.LENS_FACING_FRONT) {
                        notNull = characteristics.get(CameraCharacteristics.FLASH_INFO_AVAILABLE).booleanValue();
                        break;
                    }
                }
            }
    ...
    public static Size getPictureSize(List<Size> choices, @CameraConfiguration.MediaQuality int mediaQuality) {
        ...
        if (mediaQuality == CameraConfiguration.MEDIA_QUALITY_HIGHEST) {
            List<Size> temp = choices;
            temp.remove(maxPictureSize);
            result = Collections.max(temp, new CompareSizesByArea2());
    ...
    @SuppressWarnings("deprecation")
    public static Size getSizeWithClosestRatio(List<Size> sizes, int w, int h) {
...
//                return size;
    ...
    public static CamcorderProfile getCamcorderProfile(int currentCameraId, long maximumFileSize, int minimumDurationInSeconds) {
        ...
        int[] qualities = new int[]{CameraConfiguration.MEDIA_QUALITY_HIGHEST, CameraConfiguration.MEDIA_QUALITY_HIGH, CameraConfiguration.MEDIA_QUALITY_MEDIUM, CameraConfiguration
                .MEDIA_QUALITY_LOW, CameraConfiguration.MEDIA_QUALITY_LOWEST};
