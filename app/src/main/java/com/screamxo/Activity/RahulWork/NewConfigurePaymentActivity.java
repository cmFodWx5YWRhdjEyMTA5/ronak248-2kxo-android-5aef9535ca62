...
import com.example.apimodule.ApiBase.ApiBean.ErrorBody;
...
public class NewConfigurePaymentActivity extends AppCompatActivity implements CommonMethod, FragmentManager.OnBackStackChangedListener, SelectPaymentFragment.OnPaymentSelectedListener {

    public static final int REQ_CODE_FRIENDS_ACTIVITY_RESULTS = 101;
    private static final String TAG = "NewConfigurePaymentActivity";
    ...
    private ArrayList<SalesUser> arrayListSalesUser = new ArrayList<>();
    private ArrayList<String> arrayListSalesName = new ArrayList<>();
    String sppinner_value;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "onCreate: ");
        setContentView(R.layout.activity_new_configure_payment);
        ...
        sp_users.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                sppinner_value = arrayListSalesUser.get(i).getUsername();
            }
    ...
    public void init() {
        ...
        if (BundleUtils.getIntentExtra(getIntent(), "screen", "").equalsIgnoreCase("BoostActivity")) {
            callgetSalesUserListApi();
        }


    }

    private void setSpinnerUsers() {
        final ArrayAdapter<String> spinnerArrayAdapter = new ArrayAdapter<String>(
                this, R.layout.spinner_item, arrayListSalesName);

        spinnerArrayAdapter.setDropDownViewResource(R.layout.spinner_item);
        sp_users.setAdapter(spinnerArrayAdapter);
    }

    private void callgetSalesUserListApi() {
        if (Utils.isInternetOn(this)) {
            Call<ArrayList<SalesUser>> uploadBoostCall =
                    mservice.getBoostFetcherService(this, false).getSalesUser();
            uploadBoostCall.enqueue(new Callback<ArrayList<SalesUser>>() {
                @Override
                public void onResponse(Call<ArrayList<SalesUser>> call, Response<ArrayList<SalesUser>> response) {
                    arrayListSalesUser.addAll(response.body());
                    sppinner_value = arrayListSalesUser.get(0).getUsername();

                    for (int i = 0; i < arrayListSalesUser.size(); i++) {
                        arrayListSalesName.add(arrayListSalesUser.get(i).getName());

                    }
                }

                @Override
                public void onFailure(retrofit2.Call<ArrayList<SalesUser>> call, Throwable t) {
    ...
    public void callSucessBoostApi() {
        String item_name = BundleUtils.getIntentExtra(getIntent(), "item_name", null);
        ;
        String itemImage = BundleUtils.getIntentExtra(getIntent(), "item_image", null);
        if (Utils.isInternetOn(this)) {
            Call<JsonElement> uploadBoostCall =
                    mservice.getBoostFetcherService(this, false)
                            .uploadBoost("", 1, 1
                                    , "http://www.google.com", item_name, itemImage, 1, "100", "7");
            uploadBoostCall.enqueue(new Callback<JsonElement>() {
                @Override
                public void onResponse(Call<JsonElement> call, Response<JsonElement> response) {
                    android.widget.Toast.makeText(NewConfigurePaymentActivity.this, "sucess", android.widget.Toast.LENGTH_SHORT).show();
                }
    ...
    public void initControlValue() {
        ...
        frmItemQuantity.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                int quan = Integer.parseInt(BundleUtils.getIntentExtra(getIntent(), "total_quantity", "0"));
                if (quan > 500)
                    quan = 500;
                ...
                DialogFragmentForItemQuantity dialogFragment = new DialogFragmentForItemQuantity(NewConfigurePaymentActivity.this, qtyList, context);
        ...
        ll_wallet_container.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                startActivityForResult(new Intent(NewConfigurePaymentActivity.this, WalletSendReceiveActivity.class), REQ_CODE_WALLET_ACTIVITY_RESULTS);
            }
        });
...
//          txt_quntity.setText(BundleUtils.getIntentExtra(getIntent(), "selected_quantity", ""));
            ...
            txt_shipping_cost_value.setText("Free");
    ...
    private void setTitle() {
        ...
        if (fragment != null && fragment instanceof SelectPaymentFragment) {
            txtToolbarTitle.setText(getString(R.string.txt_cap_select_payment));
            return;
        }

        if (fragment != null && fragment instanceof SelectProcessorFragment) {
            txtToolbarTitle.setText(getString(R.string.txt_cap_select_processor));
            return;
        }
        ...
        if (BundleUtils.getIntentExtra(getIntent(), "screen", "").equalsIgnoreCase(ItemDetailsAcitvity.class.getSimpleName())
                || BundleUtils.getIntentExtra(getIntent(), "screen", "").equalsIgnoreCase(DashboardAdapter.class.getSimpleName())
                || BundleUtils.getIntentExtra(getIntent(), "screen", "").equalsIgnoreCase(ShopFragmentView.class.getSimpleName())) {
            txtToolbarTitle.setText(getString(R.string.txt_cap_payment));
            return;
        }
    ...
    private void initFabIcon() {
        ...
        floatingButton.setStateChangeListener(new FloatingMenuStateChangeListener() {
            ...
            public void onMenuOpened(FloatingMenuButton floatingMenuButton) {
                ...
                Utils.hideKeyboard(NewConfigurePaymentActivity.this);
        ...
        sbChat.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                floatingButton.closeMenu();
                if (!preferences.getUserId().isEmpty() && !preferences.getStripeCustomerId().isEmpty()) {
                    Intent gotoNext = new Intent(NewConfigurePaymentActivity.this, UploadDataActivity.class);
        ...
        sbSearch.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                floatingButton.closeMenu();
                if (!preferences.getUserId().isEmpty() && !preferences.getStripeCustomerId().isEmpty()) {
                    Intent gotoNext = new Intent(NewConfigurePaymentActivity.this, FriendsActivity.class);
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_continue:
                if (!TextUtils.isEmpty(tv_shipping_address.getText().toString())) {
                    ...
                    if (BundleUtils.getIntentExtra(getIntent(), "screen", "").equalsIgnoreCase(TopUpActivity.class.getSimpleName())) {
                        if (tv_selected_payment.getVisibility() == View.VISIBLE) {
                            DialogBox.showDialog(context, context.getString(R.string.app_name),
                                    getString(R.string.msg_select_card_to_topup), DialogBox.DIALOG_FAILURE, null);
                            return;
                        }
                    ...
                    if (BundleUtils.getIntentExtra(getIntent(), "screen", "").equalsIgnoreCase(SettingFragment.class.getSimpleName())) {
                        startActivityForResult(new Intent(NewConfigurePaymentActivity.this, WalletSendReceiveActivity.class), REQ_CODE_TOP_UP_ACTIVITY_RESULTS);
                        finish();
                    }
                } else {
...
//                            Utils.showToast(context, response.body().getMsg());
            ...
            if (Double.parseDouble(preferences.getAmount()) < totalCostValueStr.doubleValue()) {
                ...
                builder.setTitle("2KXO")
                        ...
                        .setPositiveButton(getString(R.string.txt_add_money), new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                startActivityForResult(new Intent(NewConfigurePaymentActivity.this, TopUpActivity.class), REQ_CODE_TOP_UP_ACTIVITY_RESULTS);
                                dialog.dismiss();
                            }
                        })
    ...
    private void payUsingWalletforMultipleItem() {
        ...
        try {
            ...
            if (Double.parseDouble(preferences.getAmount()) < Double.parseDouble(totalCostValueStr)) {
                ...
                JsonObject jsonMultipleItem = new JsonObject();
                jsonMultipleItem.addProperty("item_id", BundleUtils.getIntentExtra(getIntent(), "itemid", ""));
                jsonMultipleItem.addProperty("productqty", txt_quntity.getText().toString());
                multipleItemarray.add(jsonMultipleItem);
                    ...
                    if (response.code() == StaticConstant.RESULT_OK) {
                        if (response.body().getStatus().equals(StaticConstant.STATUS_1)) {
                            ...
                            if (response.body().getResult().getApproveLink() != null) {
                                Intent intent = new Intent(NewConfigurePaymentActivity.this, PayPalVerificationActivity.class);
                                ...
                                Toast.makeText(context, getString(R.string.txt_not_getting_payment_link), Toast.LENGTH_SHORT).show();
                            }
    ...
    private void returnResultAfterSuccessfullPayment(Response<FinalProcessResponse> response) {
        ...
        */
        ...
        returnIntent.putExtra("itemid", BundleUtils.getIntentExtra(getIntent(), "itemid", null));
        returnIntent.putExtra("item_name", BundleUtils.getIntentExtra(getIntent(), "item_name", null));
        returnIntent.putExtra("item_image", BundleUtils.getIntentExtra(getIntent(), "item_media_thumb", null));
        setResult(RESULT_OK, returnIntent);
        finish();
    }
            ...
            boostUsingWalletCall.enqueue(new Callback<StripeToken>() {
                ...
                public void onResponse(Call<StripeToken> call, Response<StripeToken> response) {
                    ...
                    if (response.code() == StaticConstant.RESULT_OK) {
                        ...
                        try {
                            errorBody = new Gson().fromJson(response.errorBody().string(), ErrorBody.class);
                            DialogBox.showDialog(context, context.getString(R.string.app_name), errorBody.getMessage(), DialogBox.DIALOG_FAILURE, null);
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }
    ...
    private void onBoostSuccess() {
        Intent gotoCOngrats = new Intent(NewConfigurePaymentActivity.this, BoostCongActivity.class);
        gotoCOngrats.putExtra("image", BundleUtils.getIntentExtra(getIntent(), "item_image", null));
        gotoCOngrats.putExtra("itemid", BundleUtils.getIntentExtra(getIntent(), "itemid", ""));
        gotoCOngrats.putExtra("boost_url", BundleUtils.getIntentExtra(getIntent(), "boost_url", ""));
        if (getIntent().getExtras().getString("videourl") != null)
            gotoCOngrats.putExtra("videourl", getIntent().getExtras().getString("videourl"));

        if (getIntent().getExtras().getString("item_name") != null)
            gotoCOngrats.putExtra("item_name", getIntent().getExtras().getString("item_name"));


        gotoCOngrats.putExtra("name", name);
        gotoCOngrats.putExtra("member_uaserName", sppinner_value);
        gotoCOngrats.putExtra("uid", preferences.getUserId());
        gotoCOngrats.putExtra("boost_type", BundleUtils.getIntentExtra(getIntent(), "boost_type", ""));
        gotoCOngrats.putExtra("no_days", BundleUtils.getIntentExtra(getIntent(), "day", ""));
        gotoCOngrats.putExtra("no_users", BundleUtils.getIntentExtra(getIntent(), "reach", ""));
...
//            HashMap<String, ArrayList<File>> fileArray = new HashMap<String, ArrayList<File>>();
            ...
            File[] file = StaticConstant.imageFiles_new;
//            fileArray.put("media", files);
           ...
           /* RequestBody propertyImage = RequestBody.create(MediaType.parse("image/*"), file);
            ...
            for (int index = 0; index < 4; index++) {
                RequestBody surveyBody = RequestBody.create(MediaType.parse("image/*"), file[index]);
                surveyImagesParts[index] = MultipartBody.Part.createFormData("media", file[index].getName(), surveyBody);
            }
